...
export default function TrackingPage() {
  ...
  function updateVehicleMarker(lat: number, lon: number, direction: number, label: string, speed: number) {
    if (!mapRef.current) return;
    const L = (window as any).L;
    if (!L) return;

    const iconColor = '#f59e0b';
    const rotationDeg = (direction || 0) - 45;

    const taxiIcon = L.divIcon({
      html: `<div style="font-size: 28px; display: flex; align-items: center; justify-content: center; width: 44px; height: 44px; background: white; border-radius: 50%; box-shadow: 0 2px 8px rgba(0,0,0,0.4); border: 2px solid ${iconColor}; transform: rotate(${rotationDeg}deg); line-height: 1;">🚖</div>`,
